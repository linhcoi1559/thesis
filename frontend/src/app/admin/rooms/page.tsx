...
export default function AdminRoomsPage() {
  ...
  const handleSaveRoom = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingRoom ? `http://localhost:3000/rooms/${editingRoom.id}` : `http://localhost:3000/rooms`;
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/rooms/${selectedRoom.id}/images`, { method: 'POST', cache: 'no-store', headers: { 'Authorization': `Bearer ${token}` }, body: fd });
      if (response.ok) {
